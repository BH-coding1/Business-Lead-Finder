...
class Google_maps_lead_scraper:
    ...
    def scrape_business_name(self):
        ...
        self.wait(1.5,2)
        try:
            for each_division in self.all_divisions:
                business_name = each_division.find_element(By.CLASS_NAME,'fontHeadlineSmall')
                self.all_business_names_list.append(business_name.text)
            print(self.all_business_names_list)
        except (ElementNotInteractableException, NoSuchElementException, StaleElementReferenceException):
            print('Error in finding element')
            pass
    ...
    def scrape_business_review(self):
        try:
            for each_division in self.all_divisions:
                self.wait(1.5,2)
                review = each_division.find_element(By.XPATH,'.//span[@role="img"]')
                self.all_reviews_list.append(review.text.split('(')[0])
                self.all_number_of_reviews_list.append(review.text.split('(')[1])
    ...
    def clean_phone(self,text):
        ...
        return text.replace('+1 ', '').replace('+1', '').strip()
